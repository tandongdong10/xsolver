...
void b_ax(int nIntCells,double *val,int *ptr,int *col_idx,double *b0,double *x0,double *b_ax){
     spmv_csr(nIntCells, nIntCells, ptr, val, col_idx, x0, b_ax);
     scal(nIntCells,-1.0,b_ax);
     axpy(nIntCells,1.0,b0,b_ax);
}
void residuumc(int n, TYPE *offdiagval, int *ptr, int *colindex, TYPE *rhs, TYPE *x, TYPE *y)
{
    ...
    {    
        ...
        {    
            colin = colindex[iside];
            r = r - offdiagval[iside] * x[colin];
        }    
        y[icell] = r; 
    }    
}
...
void my_solvebicgstab_c_(int maxiter, TYPE tol, int nIntCells, TYPE *a_p, TYPE *a_l, int *NbCell_ptr_c, int *NbCell_s, TYPE *b0, TYPE *x0, TYPE *res0, int *usediter)
{
    ...
    printf("[PROMPT] : X-solver V1.0 BICGSTAB solver begin\n");
    ...
    b_ax(nIntCells,a_l, NbCell_ptr_c,NbCell_s, b0,x0,res);
    ...
    *res0 = sqrt(*res0);
    ...
    beto = 1.0;
    gama = 1.0;
    for (iiter = 1; iiter < maxiter; iiter++)
    {
        *usediter = iiter;
        ...
        resl = sqrt(resl);
        rsm = resl / (*res0 + small);
        printf("[LOG] : iter = %d , relative error = %lf, absolute error = %lf\n",iiter,rsm,resl);
        if (rsm < tol)
        {
            *usediter = iiter;
            // applying the preconditioner
            ...
            free(reso);
            free(tmp_v);
            printf("[PROMPT] : X-solver V1.0 BICGSTAB solver end without error\n");
            return;
        }
    }
    // applying the preconditioner
 ...
 TYPE *a_l, int *NbCell_ptr_c, int *NbCell_s, TYPE *b0, TYPE *x0, TYPE *res0, int *usediter)
{
   ...
   //left preconditioned cg method with Jacobi preconditioner
    ...
    printf("[PROMPT] : X-solver V1.0 CG solver begin\n");
   TYPE *pk = (TYPE *)malloc(sizeof(TYPE) * nIntCells);
   TYPE *res = (TYPE *)malloc(sizeof(TYPE) * nIntCells);
   TYPE *zk = (TYPE *)malloc(sizeof(TYPE) * nIntCells);
   TYPE *diag = (TYPE *)malloc(sizeof(TYPE) * nIntCells);
      ...
      *usediter = iiter;
      ...
      if (rsm < tol)
      {
         *usediter = iiter;
         ...
         free(Apk);
         free(qk);
         printf("[PROMPT] : X-solver V1.0 CG solver end without error\n");
         return;
      }
   }
   *usediter = iiter;
