...
void spmv(const int rows,const int cols,const int nnz,const double* value,const int* index,const int width,const double* x,double* y){
    
    if(width > 2 * ( nnz / rows)){
        printf("[ADVISEING] : please use csr spmv because of the width is too big\n");
    }
