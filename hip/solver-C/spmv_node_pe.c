...
#pragma disjoint(*x,*y,*v1,*v2,*v3,*v4,*v5)
    ...
    for (i = 0,row = 0; i< node_max; ++i) {
        ...
        //nonzerorow += (n>0)*nsz;
        ii         += nsz;
        PrefetchBlock(idx+nsz*n,n,0,0);    /* Prefetch the indices for the block row after the current one */
        PrefetchBlock(v1+nsz*n,nsz*n,0,0); /* Prefetch the values for the block row after the current one  */
               ...
               const int node_max, const int *ns)
{
  ...
  for (i = 0,row = 0; i< node_max; ++i) {
    ...
    n           = width;
    //nonzerorow += (n>0)*nsz;
    PrefetchBlock(idx+nsz*n,n,0,PETSC_PREFETCH_HINT_NTA);    
    PrefetchBlock(v1+nsz*n,nsz*n,0,PETSC_PREFETCH_HINT_NTA); 
